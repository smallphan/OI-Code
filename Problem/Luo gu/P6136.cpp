...
int n, m, cnt, root, opt, num, ans, lst;
vector<int> vec;
...
int main() {
    n = read(), m = read();
    while (n--) modef::insert(root, read());
    while (m--) {
        opt = read(), num = read() ^ lst;
        switch (opt) {
            case 1 : modef::insert(root, num); break;
            case 2 : modef::del(root, num); break;
            case 3 : lst = modef::getrnk(root, num), ans ^= lst; break;
            case 4 : lst = modef::getval(root, num), ans ^= lst; break;
            case 5 : lst = val(modef::getpre(root, num)), ans ^= lst; break;
            case 6 : lst = val(modef::getnxt(root, num)), ans ^= lst; break;
        }
    }
    printf("%d\n", ans);
    return 0;
